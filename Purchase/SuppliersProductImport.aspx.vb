...
Partial Public Class SuppliersProductImport
    ...
    Private Function GetProductInformation(ByVal CASNumber As String) As ProductDataType
        ...
        Try
            ...
            '権限ロールに従い極秘品の情報を非表示とする
            If Session(SESSION_ROLE_CODE).ToString = ROLE_WRITE_P OrElse Session(SESSION_ROLE_CODE).ToString = ROLE_READ_P Then
                sb_SQL.AppendLine("WITH C AS (")
                sb_SQL.AppendLine("  SELECT DISTINCT")
                sb_SQL.AppendLine("    CAS")
                sb_SQL.AppendLine("  FROM")
                sb_SQL.AppendLine("    s_NewProduct")
                sb_SQL.AppendLine("  WHERE")
                sb_SQL.AppendLine("    isCONFIDENTIAL = 1")
                sb_SQL.AppendLine(")")
                sb_SQL.AppendLine("SELECT ")
                sb_SQL.AppendLine("  CASE WHEN C.CAS IS NOT NULL THEN NULL ELSE P.ProductNumber END AS ProductNumber,")
                sb_SQL.AppendLine("  CASE WHEN C.CAS IS NOT NULL THEN NULL ELSE ISNULL((SELECT E.ENai FROM s_EhsPhrase AS E WHERE E.PhID = P.[Status]), '-') END AS [Status],")
                sb_SQL.AppendLine("  CASE WHEN C.CAS IS NOT NULL THEN NULL ELSE ISNULL((SELECT E.ENai FROM s_EhsPhrase AS E WHERE E.PhID = P.ProposalDept), '-') END AS ProposalDept,")
                sb_SQL.AppendLine("  CASE WHEN C.CAS IS NOT NULL THEN NULL ELSE ISNULL((SELECT E.ENai FROM s_EhsPhrase AS E WHERE E.PhID = P.ProcumentDept), '-') END AS ProcumentDept")
                sb_SQL.AppendLine("FROM")
                sb_SQL.AppendLine("  Product AS P")
                sb_SQL.AppendLine("    LEFT OUTER JOIN C ON C.CAS = P.CASNumber")
                sb_SQL.AppendLine("WHERE")
                sb_SQL.AppendLine("  P.CASNumber = @CASNumber")
            Else
                sb_SQL.AppendLine("SELECT ")
    ...
    Private Function SetSupplierProductListDataToTable(ByVal Conn As SqlConnection) As DataTable
        ...
        If Session(SESSION_ROLE_CODE).ToString = ROLE_WRITE_P OrElse Session(SESSION_ROLE_CODE).ToString = ROLE_READ_P Then
            ...
            sb_SQL.AppendLine("    ,CASE WHEN C.CAS IS NOT NULL THEN '0' ELSE MAX(CASE WHEN T1.CASNumber IS NULL OR T2.ALDRICH IS NULL THEN '0' ELSE CONVERT(VARCHAR, T2.ALDRICH) END) END AS AD")
            sb_SQL.AppendLine("    ,CASE WHEN C.CAS IS NOT NULL THEN '0' ELSE MAX(CASE WHEN T1.CASNumber IS NULL OR T2.ALFA IS NULL THEN '0' ELSE CONVERT(VARCHAR, T2.ALFA) END) END AS AF")
            sb_SQL.AppendLine("    ,CASE WHEN C.CAS IS NOT NULL THEN '0' ELSE MAX(CASE WHEN T1.CASNumber IS NULL OR T2.WAKO IS NULL THEN '0' ELSE CONVERT(VARCHAR, T2.WAKO) END) END AS WA")
            sb_SQL.AppendLine("    ,CASE WHEN C.CAS IS NOT NULL THEN '0' ELSE MAX(CASE WHEN T1.CASNumber IS NULL OR T2.KANTO IS NULL THEN '0' ELSE CONVERT(VARCHAR, T2.KANTO) END) END AS KA")
            ...
            sb_SQL.AppendLine("  LEFT OUTER JOIN C")
            sb_SQL.AppendLine("    ON C.CAS = T1.CASNumber")
